...
int relay_request(char *host, char *method, char *filename, rio_t *connrio,
                  int clientfd);
...
/*
 * relay_reponse - read HTTP response from connfd and write them to the
 * clientrio
 */
int relay_response(rio_t *clientrio, int connfd)
{
  ...
  if (rio_writen(connfd, response, strlen(response)) < 0)
    return -1;
