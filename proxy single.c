...
void doit(int fd);
int relay_request(char *host, char *filename, rio_t *connrio, int clientfd);
 ...
 * and <filename>. Port is optional and defults to DEFAULT_HOST.
 */
void parse_uri(char *uri, char *host, char *port, char *filename)
{
...
void relay(int connfd)
{
  ...
  if (relay_request(host, filename, &connrio, clientfd) != 0)
  {
